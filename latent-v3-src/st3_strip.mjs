...
for (const [name, opts, n] of [['d',{viewport:{width:1440,height:900}},16],['m',{...devices['iPhone 13']},10]]){
  ...
  const info = await pg.evaluate(()=>{ const hero=document.querySelector('.hero').closest('.js-lk')||document.querySelector('.hero'); const a=hero.getBoundingClientRect().top+scrollY+hero.offsetHeight-innerHeight*.5; const z=document.querySelector('#method').getBoundingClientRect().top+scrollY+innerHeight*.5; return {a,z}; });
