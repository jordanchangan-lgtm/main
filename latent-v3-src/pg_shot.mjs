...
for (const [name, opts] of [['d',{viewport:{width:1440,height:900}}],['m',{...devices['iPhone 13']}]]){
  ...
  for(const f of [0.0,0.06,0.15,0.3,0.45,0.6,0.8,0.97]){ const t=info.t+(info.h-ih)*f; while(y<t){ y=Math.min(t,y+200); await pg.evaluate(v=>scrollTo(0,v),y); await pg.waitForTimeout(8);} await pg.waitForTimeout(1000); await pg.screenshot({path:`pg_${name}_${k++}.png`}); }
