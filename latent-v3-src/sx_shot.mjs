...
for (const [name, opts] of [['d',{viewport:{width:1440,height:900}}],['m',{...devices['iPhone 13']}]]){
  ...
  const tops = await pg.evaluate(()=>[...document.querySelectorAll('.js-sx-panel')].map(e=>e.getBoundingClientRect().top+scrollY));
  ...
  for(const i of [1,2,3,4]){ await pg.evaluate(v=>scrollTo(0,v), tops[i]); await pg.waitForTimeout(1900); await pg.screenshot({path:`sx_${name}_${k++}.png`}); }
