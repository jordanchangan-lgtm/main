...
for (const [name, opts] of [['d',{viewport:{width:1440,height:900}}],['m',{...devices['iPhone 13']}]]){
  ...
  for(const f of [0.12,0.3,0.5,0.75]){ await pg.evaluate(v=>scrollTo(0,v), run*f); await pg.waitForTimeout(f===0.12?250:(f===0.75?2600:900)); await pg.screenshot({path:`hero_${name}_${k++}.png`}); }
